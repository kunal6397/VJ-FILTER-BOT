...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1837294444').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002054782510').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1837294444').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel,'-1002097822007') if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', 'NewGroup4Movies')
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002097822007')
...
OWNER_USERNAME = environ.get('OWNER_USERNAME', 'i_am_yamraj') # owner username without @
...
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/NewGroup4Movies')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/NeonGhost_Networks')
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/NeonGhost_Networks')
IS_TUTORIAL = bool(environ.get('IS_TUTORIAL', True))
MSG_ALRT = environ.get('MSG_ALRT', 'Hello My Dear Friends ❤️')
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002034727890'))
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'NewGroup4Movies')
...
URL = "https://structural-cecil-neonghost.koyeb.app".format(FQDN) if ON_HEROKU or NO_PORT else \
    "https://structural-cecil-neonghost.koyeb.app/".format(FQDN, PORT)
...
if HAS_SSL:
    URL = "https://structural-cecil-neonghost.koyeb.app/".format(FQDN)
else:
    URL = "https://structural-cecil-neonghost.koyeb.app/".format(FQDN)
